...
using namespace std;
<<<<<<< HEAD
void timestamp_action() {//仅输入输出，不用优化
=======
...
void write_action() {
    ...
    for (int i = 1; i <= n_write; i++) {
        int id, size,label_id;
        // 读取对象编号和对象大小，%*d 表示忽略对象标签（或其他无用信息）
        scanf("%d%d%d", &id, &size, &label_id);
        // 初始化该对象的请求链为空
        ...
        for (int j = 0; j <3; j++) 
        {
            ...
            {
                if (label_id!=object[disk[disk_id][i1>1?i1-1:V]].label_id) continue;
                int rest_units_num=0;//从i1开始连着的空闲单元数
                for (int rest_units=i1;rest_units_num<size&&rest_units<=V;rest_units++)
                {
                    if (disk[disk_id][rest_units]) break;
                    rest_units_num++;
                }
                if (rest_units_num==size)
                {
                    for (int j1=0;j1<size;j1++)
                    {
                        disk[disk_id][i1+j1] = id;
                        object[id].unit[j+1][++current_write_point] = i1+j1;
                        disk_uid[disk_id][i1+j1]=current_write_point;
                    }
                    break;
                }
            }
            if (!current_write_point)
            {
                for (int i1=1;i1<=V;i1++)
                {
                    int rest_units_num=0;//从i1开始连着的空闲单元数
...
<<<<<<< HEAD
        ...
        for (int k=1;k<=object[object_id].size;k++)
        {
            ...
            for (int j=1;j<=3;j++)
            {
                ...
                if (disk_vector[mn].size()>disk_vector[now].size()) d=j; //**按待处理单元数判断磁盘优劣
            }
    ...
    for (int i = 1; i <= N; i++) 
    {
        ...
        if (!pre_time) pre_time=100;//没有给last_time赋初始值，当它是0时说明之前没有用过
        
        //**处理jump----------------------------------------------------- 
        if (!disk_vector[i].size()) continue;
        ...
        while (rest_time)
        {
            if (!disk_vector[i].size()) break;//这个磁盘没有待读的单元了就退出
            int to=cal_to_pos(i,ptr[i]+1);//**读取顺序策略是不管进入容器顺序，优先读取距离最近的
            int dis=to-ptr[i]-1;//距离目标单元的距离
            if (dis<0) dis+=V;
            if (!dis)//针就在要读的位置
            {
                ...
                if (cost_time>rest_time) rest_time=0;//不够时间读，又不能走，直接结束时间片
                else
                {
                    rest_time-=cost_time;
                    res+="r";
